...
%m_adj = ~m_adj_conf; % we use the inverse conflict graph
...
for t = 1:3*max_t
    ...
    sum_y = sum_neighbors(rest_neighbors, l_ij(:,orig));
    y_ij(orig,dest) = max(w(orig)-sum_y,0);
    ...
    sum_y = sum_neighbors(rest_neighbors, l_ij(:,dest));
    y_ij(dest,orig) = max(w(dest)-sum_y,0);
    % Calculate lambda
    a = y_ij(orig,dest);
    b = y_ij(dest,orig);
...
[~,nodes_sorted] = sort(w, 'descend');
for ii = 1:length(nodes_sorted)
    i = nodes_sorted(ii);
    if colour(i) == 1
