function [links_norm, v_dist] = clustered_network(space, n_nodes, n_clusters, linked_nodes)
...
% % Every centroid will be connected by fiber
% links_fib = [];
% for i=1:n_clusters
%     for j=1:n_clusters
%         if i ~= j
...
adj_mat = zeros(n_nodes+n_clusters,n_nodes+n_clusters);
...
hold on;
plot(nodes_pos(:,1),nodes_pos(:,2),'kx');
plot(cent_pos(:,1),cent_pos(:,2),'r+');
axis([0 space(1), 0 space(2)])
legend('Links','Users', 'Centroid');
grid on;
