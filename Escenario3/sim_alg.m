function [mean_rij, a, p_mean, Opt, V, J, C] = sim_alg (sim, isDistr, isBatt, showFig)
...
for i = 1:total_duration
    ...
    if isBatt
        for m = 1:n_nodes
           % Collect energy
           [e_op(m), e_out(m), e_in(m)] = harvesting_algorithm(pot_tot(m), nrg_harv(m,i), nrg_cost(i), Batt_cap(m), rec_batt(m,i), pi(m));
        ...
        A_Interferemce_r = 0;
         for m = 1:n_nodes
             if abs(G_est(i,k,m))^2 > 0
                  A_Interferemce = A_Interferemce + Average_prim(k,1) * Active_s(m) * pot_tot(m) * abs(G_est(i,k,m))^2;
                  A_Interferemce_r = A_Interferemce_r + Average_prim(k,1) * Active_s(m) * pot_tot(m) * abs(G(i,k,m))^2;
             end;
         end;
         acum_inter (i,k) = A_Interferemce_r;
         th(k, 1) = max(th(k,1) + pass_th * (A_Interferemce  - Max_Interference),0);
    end;
...
if showFig
    ...
    hold on; 
    plot(t, rec_pi(3,t), 'r'); 
    title('Comparativa entre el coste de comprar potencia y \Pi');
    legend('Coste','\Pi');

    figure(3);
    plot(t, rec_p(3,t), 'k');
    hold on; 
    plot(t, rec_e_op(3,t), 'r'); 
    plot(t, rec_e_out(3,t), 'b'); 
    legend('P_i', 'P_\op','E_\out');
    title('Potencia comprada vs Potencia sacada de la batería');

    figure(4);
    plot(t,rec_batt(3,t), 'b');
